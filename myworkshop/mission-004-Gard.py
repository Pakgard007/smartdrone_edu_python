        ...
        while (True):
            try :
                ...
                if minput == '1':
                    ...
                    with fly.sync_these():
                        fly.curve(x1=30, y1=0, z1=50, x2=60, y2=80, z2=100, speed=40,tello=1)

                    with fly.sync_these():
                        fly.straight( x=200, y=0, z=-70, speed=40, tello=1)

                    with fly.sync_these():
                        fly.up(dist=80, tello=1)

                    with fly.sync_these():
                        fly.straight( x=-200, y=0, z=-70, speed=40, tello=1)
                    
                    with fly.sync_these():
                        fly.right(dist=70, tello=1)

                    with fly.sync_these():
                        fly.back(dist=70, tello=1)

                    with fly.sync_these():
                        fly.left(dist=70, tello=1)    

                    with fly.sync_these():
                        fly.forward(dist=70, tello=1) 
